...
<title>@yield('title','SIJAMU') — Sistem Informasi Jadwal Tamu | Gedung Wijaya</title>
...
<header class="topbar">
  ...
  <nav class="topbar-nav">
    <a href="{{ route('home') }}" class="tnl {{ request()->routeIs('home') ? 'active' : '' }}">Beranda</a>
    <a href="{{ route('reservasi.form') }}" class="tnl {{ request()->routeIs('reservasi.*') ? 'active' : '' }}">Buat Reservasi</a>
    <a href="{{ route('cek.form') }}" class="tnl {{ request()->routeIs('cek.*') ? 'active' : '' }}">Cek Status</a>
  </nav>
  <div class="topbar-actions">
    <a href="{{ route('admin.login') }}" class="btn btn-secondary btn-sm">Masuk Admin</a>
  </div>
</header>
...
<nav class="mobile-nav">
  <div class="mobile-nav-list">
    ...
    <a href="{{ route('reservasi.form') }}" class="mobile-nav-link {{ request()->routeIs('reservasi.*') ? 'active' : '' }}">📝 Buat Reservasi</a>
    <a href="{{ route('cek.form') }}" class="mobile-nav-link {{ request()->routeIs('cek.*') ? 'active' : '' }}">🔍 Cek Status</a>
    <div class="mobile-nav-divider"></div>
    <a href="{{ route('admin.login') }}" class="mobile-nav-link">⚙️ Masuk Admin</a>
  </div>
...
<main class="cw">
  @if(session('success'))
    <div class="flash"><div class="alert alert-ok"><span class="ai">✓</span>{{ session('success') }}</div></div>
  @endif
  @if(session('error'))
    <div class="flash"><div class="alert alert-err"><span class="ai">✕</span>{{ session('error') }}</div></div>
  @endif
...
<footer class="site-footer">
  <p>&copy; {{ date('Y') }} <strong>Pemerintah Kabupaten Sukoharjo</strong> &mdash; SIJAMU v2.0 &nbsp;|&nbsp; Gedung Wijaya, Jl. Jenderal Sudirman No. 199, Sukoharjo 57511</p>
</footer>
