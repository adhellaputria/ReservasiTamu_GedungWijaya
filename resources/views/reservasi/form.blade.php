...
<div class="form-pg">
  ...
  <form action="{{ route('reservasi.store') }}" method="POST" enctype="multipart/form-data" id="formReservasi" novalidate>
    @csrf
      ...
      <div class="card-body">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="text" name="nama_tamu" id="nama_tamu" class="fc {{ $errors->has('nama_tamu') ? 'is-invalid' : '' }}" value="{{ old('nama_tamu') }}" placeholder="Masukkan nama lengkap Anda">
            <div class="invalid-feedback">{{ $errors->first('nama_tamu') }}</div>
          </div>
          <div class="form-group">
            <label class="form-label">Nomor HP / WhatsApp <span class="req">*</span></label>
            <input type="tel" name="no_hp" id="no_hp" class="fc {{ $errors->has('no_hp') ? 'is-invalid' : '' }}" value="{{ old('no_hp') }}" placeholder="08123456789">
            <div class="invalid-feedback">{{ $errors->first('no_hp') }}</div>
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Alamat Email <span class="req">*</span></label>
          <input type="email" name="email_tamu" id="email_tamu" class="fc {{ $errors->has('email_tamu') ? 'is-invalid' : '' }}" value="{{ old('email_tamu') }}" placeholder="nama@email.com">
          <div class="invalid-feedback">{{ $errors->first('email_tamu') }}</div>
        </div>
        <div class="form-group">
          <label class="form-label">Instansi / Lembaga / Asal</label>
          <input type="text" name="instansi" class="fc" value="{{ old('instansi') }}" placeholder="Nama instansi atau lembaga (opsional)">
        </div>
        <hr class="div">
        <div class="fsec-ttl">Keperluan</div>
        <div class="form-group">
          <label class="form-label">Ringkasan Tujuan Kunjungan <span class="req">*</span></label>
          <input type="text" name="tujuan" id="tujuan" class="fc {{ $errors->has('tujuan') ? 'is-invalid' : '' }}" value="{{ old('tujuan') }}" placeholder="Contoh: Konsultasi perizinan usaha bidang perdagangan">
      ...
      <div class="card-body">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <select name="opd_id" id="opd_id" class="fsel {{ $errors->has('opd_id') ? 'is-invalid' : '' }}">
              <option value="">— Pilih OPD Tujuan —</option>
              @foreach($opdList as $opd)
                <option value="{{ $opd->id }}" {{ old('opd_id') == $opd->id ? 'selected' : '' }}>
                  {{ $opd->nama }} (Lantai {{ $opd->lantai }})
                </option>
              @endforeach
            </select>
            <div class="invalid-feedback">{{ $errors->first('opd_id') }}</div>
          </div>
          <div class="form-group">
            <label class="form-label">Pejabat / Staf yang Dituju</label>
            <input type="text" name="petugas_dituju" class="fc" value="{{ old('petugas_dituju') }}" placeholder="Nama atau jabatan (opsional)">
          </div>
        </div>
        <hr class="div">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="date" name="tanggal" id="tanggal" class="fc {{ $errors->has('tanggal') ? 'is-invalid' : '' }}" value="{{ old('tanggal') }}" min="{{ date('Y-m-d') }}">
          ...
          <div class="form-group">
            ...
            <input type="time" 
                name="jam_kunjungan" 
                id="jam_kunjungan"
                class="fc {{ $errors->has('jam_kunjungan') ? 'is-invalid' : '' }}"
                value="{{ old('jam_kunjungan') }}"
                step="3600"
                required>

          <div class="invalid-feedback">{{ $errors->first('jam_kunjungan') }}</div>
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Keterangan Tambahan</label>
          <textarea name="keterangan" class="fta" placeholder="Informasi pendukung yang perlu diketahui admin OPD...">{{ old('keterangan') }}</textarea>
        </div>
      </div>
      <div class="card-footer">
    ...
    <div id="step3" class="card" style="display:none">
      ...
      <div class="card-body">
        ...
        @error('dokumen')<div class="alert alert-err mt8"><span class="ai">✕</span>{{ $message }}</div>@enderror
        <hr class="div">
